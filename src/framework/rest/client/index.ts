...
class Client {
  ...
  myReports = {
    all: (params: MyReportsQueryOptions) =>
      ...
      sendmail : (input:EmailRequestDto) =>
      HttpClient.post<any>(API_ENDPOINTS.MY_REPORTS, input)
  ...
  users = {
    ...
    getuser:(id : number) => HttpClient.get<User>(`${API_ENDPOINTS.USERS_ME}/${id}`),
  ...
  settings = {
    ...
    profileimgpath: (imagname?: string) =>
      HttpClient.get<any>(`${API_ENDPOINTS.UPLOADS}/${imagname}`),
    upload: (input: File[]) => {
  ...
  paymentgateway ={
    createorder: (input: RazorpayOrderInput) =>
    HttpClient.post<any>(API_ENDPOINTS.CREATE_ORDER_RZPAY, input),

    createpaymentinfo: (input: RazorpayPaymentInfoInput) =>
    HttpClient.post<any>(API_ENDPOINTS.RZAPAY_PAYMENTINFO, input)
