...
export function useUser() {
  ...
  const tokenUserId=Cookies.get(AUTH_TOKEN_KEY);
  ...
  const { data, isLoading, error } = useQuery<User, Error>(
    [API_ENDPOINTS.USERS_ME, Number(tokenUserId)],
    () => client.users.getuser(Number(tokenUserId))
  );
...
export function useRegister() {
  ...
  const { mutate, isLoading } = useMutation(client.users.register, {
    onSuccess: (data) => {
      ...
      if(data?.token==="-1")
      {
      toast.error(t('text-already-account'));
      return ;
      }
